...
def smoke() -> None:
    ...
    print("\n========== 起 ray head ==========")
    # 被抢占后 Modal 可能在同一容器重试，上一轮的 ray head 仍占着 6379 端口，
    # 直接 ray start 会因端口冲突硬失败。先 stop 让启动幂等（未运行时 no-op）。
...
def learn(num_rollout: int = 30, task: str = "translate", no_offload: bool = False, simul: bool = False, tag: str = "") -> None:
    ...
    if no_offload:
        # 2a 实验：colocate 不 offload、两端常驻。用独立脚本 + 独立 save_dir，
        # 完全不动已实现基线(smoke 脚本 / s2tt_probe ckpt)。
        smoke_script = "scripts/training/multimodal/run-qwen3-30B-A3B-omni-lora-noffload.sh"
        if save_dir:
            save_dir = save_dir.rstrip("/") + "_noffload"

    if simul:
        # 同传实验：整段音频按 960ms 切块的多轮 rollout（自定义 generate）。
        # 独立脚本(挂 --custom-generate-function-path)+ 独立 save_dir，不动 s2tt 基线。
        # 数据/reward 与 s2tt 一致（整段音频，切块在 env 里；reward 整段 BLEU）。
        smoke_script = "scripts/training/multimodal/run-qwen3-30B-A3B-omni-lora-simul.sh"
        if save_dir:
            save_dir = save_dir.rstrip("/") + "_simul"

    if tag and save_dir:
        # 用独立 ckpt 目录起「全新」一跑，不与旧历史续跑（也不动旧 ckpt）。
        save_dir = save_dir.rstrip("/") + f"_{tag}"
...
@app.local_entrypoint()
def learn_audio_noffload(steps: int = 5) -> None:
    """2a 实验：colocate 不 offload（两端常驻）显存/可行性验证。

    - 用独立脚本 run-...-noffload.sh（--no-offload-train/rollout + 调低 sglang mem）
      与独立 ckpt 目录 /s2tt/ckpt/s2tt_probe_noffload，不动已实现基线。
    - 默认只跑 5 步：先看是否 OOM、每步是否省掉 offload/onload；OK 再加步数。
    """
    fc = learn.spawn(num_rollout=steps, task="s2tt", no_offload=True)
    print(f"2a(不 offload)已 spawn（{steps} 步），function call ID: {fc.object_id}")
    print("看日志：modal app logs relax-omni-lora-smoke   （或 dashboard）")


@app.local_entrypoint()
def learn_simul(steps: int = 5, tag: str = "") -> None:
    """同传实验：整段音频按 960ms 切块的多轮 rollout（自定义 generate）验证。

    - 独立脚本 run-...-simul.sh（挂 examples.simul_s2tt.rollout.generate）
      与独立 ckpt 目录 /s2tt/ckpt/s2tt_probe_simul，不动已实现的 s2tt 基线。
    - 默认只跑 5 步：先看多轮切块能否跑通、reward 是否正常（整段 BLEU）。
    """
    fc = learn.spawn(num_rollout=steps, task="s2tt", simul=True, tag=tag)
    print(f"同传(多轮切块)已 spawn（{steps} 步，tag={tag or '无'}），function call ID: {fc.object_id}")
    print("看日志：modal app logs relax-omni-lora-smoke   （或 dashboard）")


# ---------------------------------------------------------------------------
# 同传 rollout 离线最小自测（CPU，约 1 分钟，不起训练/sglang/GPU）
# ---------------------------------------------------------------------------
def _offline_build_simul_out(sr: int = 16000, dur_s: float = 3.0):
    """离线构建 1 条同传样本并跑完整多轮 generate()（CPU，mock sglang），返回 ctx。

    走【真实数据构建路径】process_raw_sample 造合成音频样本（默认 3s/960ms=4 块，末块偏短
    → 复现变长音频特征拼接），mock 掉 GenerateState/post，跑 examples.simul_s2tt.rollout.generate。
    selftest_simul_encode（校验编码不变式）与 selftest_rope_index（复现/验证 device 补丁）共用它。

    返回 dict：out（生成后的 Sample）/ tokenizer / processor / sr / ct_kwargs /
             fake_tokens / fake_text / call_count / exp_prompt_len。
    """
    import asyncio
    ...
    from relax.utils.data.data_utils import process_raw_sample

    print("[offline] loading tokenizer/processor (CPU) ...", flush=True)
    tokenizer = AutoTokenizer.from_pretrained(MODEL_DIR, trust_remote_code=True)
    processor = AutoProcessor.from_pretrained(MODEL_DIR, trust_remote_code=True)

    # 合成音频 -> wav（走与训练一致的加载路径）
    t = np.linspace(0, dur_s, int(sr * dur_s), endpoint=False, dtype=np.float32)
    wav = (0.02 * np.sin(2 * np.pi * 220.0 * t)).astype(np.float32)
    wav_path = "/tmp/selftest_simul.wav"
    pcm = (wav.clip(-1, 1) * 32767).astype("int16")
    ...
    # Qwen3-Omni 的 chat_template 在 processor 里，裸 tokenizer 加载不到 -> 显式喂模板（与真机一致）
    ct_kwargs = _json.loads(_chat_template_kwargs())
    ...
    raw = {
        ...
        "audios": [wav_path],
        "label": {"ground_truth": "测试参考译文"},
        "metadata": {"src_lang": "en", "tgt_lang": "zh", "rm_type": "bleu"},
    }
    sample = process_raw_sample(
        ...
        apply_chat_template_kwargs=ct_kwargs,
    )

    import examples.simul_s2tt.rollout as R
    ...
    R.GenerateState = _FakeState

    fake_tokens = tokenizer.encode("你好，世界", add_special_tokens=False)[:4] or [1, 2, 3, 4]
    fake_text = tokenizer.decode(fake_tokens)
    call_counter = {"n": 0}

    async def _fake_post(url, payload):
        call_counter["n"] += 1
        otl = [[-0.05 * (i + 1), tok] for i, tok in enumerate(fake_tokens)]
        return {
            "text": fake_text,
            "meta_info": {"output_token_logprobs": otl, "finish_reason": {"type": "stop"}},
        }
    ...
    print("[offline] running generate() ...", flush=True)
    out = asyncio.run(R.generate(args, sample, sampling_params))

    exp_prompt = processor(
        text=out.prompt,
        return_mm_token_type_ids=False,
        use_audio_in_video=False,
        images=None,
        videos=None,
        audio=out.multimodal_inputs["audio"],
    )["input_ids"][0]
    ...
    return {
        ...
        "fake_text": fake_text,
        "call_count": call_counter["n"],
        "exp_prompt_len": len(exp_prompt),
    }


@app.function(image=image, volumes={"/models": model_volume}, timeout=15 * 60)
def selftest_simul_encode() -> None:
    """同传 rollout 的【离线最小测试】。

    目的：把「单轮→多轮」新增的编码/多轮编排逻辑在 CPU 上跑一遍，
    catch 掉此前踩过的坑（images=[] 越界、音频特征变长 cat 维度不匹配、双 token 流对齐），
    而无需起 4×A100 + 30B + sglang + Ray（那要 ~12 分钟才撞一个编码层错）。
    """
    from relax.utils.types import Sample

    ctx = _offline_build_simul_out()
    out = ctx["out"]
    fake_tokens = ctx["fake_tokens"]
    fake_text = ctx["fake_text"]
    call_counter = {"n": ctx["call_count"]}

    # ---- 断言不变式 ----
    exp_prompt_len = ctx["exp_prompt_len"]
    num_chunks = out.metadata.get("simul_num_chunks")
    gen_total = len(fake_tokens) * (num_chunks or 0)
    mm = out.multimodal_train_inputs or {}
    ...
    _chk("status==COMPLETED", out.status == Sample.Status.COMPLETED, out.status)
    _chk(
        "stop==chunks_exhausted",
        out.metadata.get("simul_stop_reason") == "chunks_exhausted",
        out.metadata.get("simul_stop_reason"),
    )
    _chk("num_chunks(>=2)", (num_chunks or 0) >= 2, num_chunks)
    _chk(
        "sglang_calls==num_chunks",
        call_counter["n"] == num_chunks,
        f"calls={call_counter['n']} chunks={num_chunks}",
    )
    _chk(
        "loss_mask_len==response_length",
        len(out.loss_mask) == out.response_length,
        f"{len(out.loss_mask)} vs {out.response_length}",
    )
    _chk(
        "logprobs_len==response_length",
        len(out.rollout_log_probs) == out.response_length,
        f"{len(out.rollout_log_probs)} vs {out.response_length}",
    )
    _chk(
        "tokens==exp_prompt+response",
        len(out.tokens) == exp_prompt_len + out.response_length,
        f"tokens={len(out.tokens)} exp_prompt={exp_prompt_len} resp={out.response_length}",
    )
    _chk("sum(loss_mask==1)==generated", sum(out.loss_mask) == gen_total, f"{sum(out.loss_mask)} vs {gen_total}")
    _chk("response==concat(gen_text)", out.response == fake_text * (num_chunks or 0), repr(out.response[:40]))
    _chk("has input_features", "input_features" in mm, list(mm.keys()))
    if "input_features" in mm:
        _chk(
            "input_features rows==num_chunks",
            mm["input_features"].shape[0] == num_chunks,
            tuple(mm["input_features"].shape),
        )
    if "feature_attention_mask" in mm:
        _chk(
            "feat_mask rows==num_chunks",
            mm["feature_attention_mask"].shape[0] == num_chunks,
            tuple(mm["feature_attention_mask"].shape),
        )

    print("\n==== SELFTEST RESULTS ====", flush=True)
    all_ok = True
    for name, ok, detail in checks:
        all_ok = all_ok and ok
        print(f"  [{'PASS' if ok else 'FAIL'}] {name}   {detail}", flush=True)
    print(
        f"\nnum_chunks={num_chunks}  tokens={len(out.tokens)}  rollout_tokens={len(out.rollout_tokens)}  "
        f"response_length={out.response_length}",
        flush=True,
    )
    if "input_features" in mm:
        fam = mm.get("feature_attention_mask")
        print(
            f"input_features={tuple(mm['input_features'].shape)}  "
            f"feature_attention_mask={tuple(fam.shape) if fam is not None else None}",
            flush=True,
        )
    if not all_ok:
        raise SystemExit("[selftest] FAILED — 见上面 FAIL 项")
    print("[selftest] ALL PASSED ✅", flush=True)


@app.local_entrypoint()
def selftest_simul() -> None:
    """离线跑同传 rollout 的最小自测（CPU，约 1 分钟），过了再上 GPU 跑真机。"""
    selftest_simul_encode.remote()


@app.function(image=image, gpu="T4", volumes={"/models": model_volume}, timeout=20 * 60)
def selftest_rope_index() -> None:
    """训练侧「多段音频位置编码」的【最小 GPU 测试】(T4，约 1-2 分钟)。

    这个 device bug 只能在 GPU 复现（全 CPU 就没有 cpu/cuda 混用），但**不需要** 30B/sglang/训练：
    get_rope_index 是纯函数，直接喂「含 4 段音频的合成序列」即可在几秒内：
      ① 用原函数复现 'cuda and cpu' 崩溃；
      ② 用生产补丁 patch_qwen3_omni_rope_index_device() 验证修好（位置 id 仍在 GPU、shape 正确）。
    """
    import torch

    if not torch.cuda.is_available():
        raise SystemExit("[rope] 需要 GPU 才能复现 device bug")
    dev = torch.device("cuda:0")

    # ---- 1. 离线造「含 4 段音频」的训练序列（复用同传 rollout 的真实编码路径）----
    ctx = _offline_build_simul_out()
    out = ctx["out"]
    mm = out.multimodal_train_inputs or {}
    assert "feature_attention_mask" in mm, f"缺 feature_attention_mask，keys={list(mm.keys())}"
    num_audios = mm["feature_attention_mask"].shape[0]
    print(f"[rope] 序列 token 数={len(out.tokens)}，音频段数={num_audios}", flush=True)

    # ---- 2. 从 config 取真实 token id（get_rope_index 靠它们解析序列）----
    from transformers import AutoConfig

    cfg_dict = AutoConfig.from_pretrained(MODEL_DIR, trust_remote_code=True).to_dict()
    ...
    print(f"[rope] config ids = {ids}", flush=True)
    for k in ("audio_token_id", "audio_start_token_id"):
        if ids[k] is None:
            raise SystemExit(f"[rope] 从 config 找不到 {k}，无法构造测试")

    # ---- 3. 复现训练 forward 里的调用（模仿 model.py:195/347）----
    input_ids = torch.tensor([out.tokens], dtype=torch.long, device=dev)
    feat_mask = mm["feature_attention_mask"].to(dev)
    audio_seqlens_cuda = feat_mask.sum(dim=1)  # 与 model.py:195 一致 → CUDA

    call_kwargs = dict(
        spatial_merge_size=ids["spatial_merge_size"] or 2,
        image_token_id=ids["image_token_id"] if ids["image_token_id"] is not None else -1,
        video_token_id=ids["video_token_id"] if ids["video_token_id"] is not None else -1,
        audio_token_id=ids["audio_token_id"],
        vision_start_token_id=ids["vision_start_token_id"] if ids["vision_start_token_id"] is not None else -1,
        audio_start_token_id=ids["audio_start_token_id"],
        input_ids=input_ids,
        image_grid_thw=None,
        video_grid_thw=None,
        attention_mask=None,
        ...
        position_id_per_seconds=ids["position_id_per_seconds"] or 25,
    )

    from megatron.bridge.models.qwen_omni.modelling_qwen3_omni import model as omni_model
    from megatron.bridge.models.qwen_omni.modelling_qwen3_omni import utils as omni_utils

    # ---- ① 复现：原函数（未打补丁）+ CUDA audio_seqlens ----
    print("\n[rope] ① 复现（原函数，audio_seqlens=CUDA）...", flush=True)
    reproduced = False
    try:
        omni_utils.get_rope_index(audio_seqlens=audio_seqlens_cuda, **call_kwargs)
        print("  [WARN] 原函数未报错（该 torch 版本行为可能不同）", flush=True)
    except RuntimeError as e:
        reproduced = "cuda" in str(e).lower() and "cpu" in str(e).lower()
        print(f"  [OK] 原函数崩溃：{str(e).splitlines()[-1][:120]}", flush=True)

    # ---- ② 验证补丁：生产 monkeypatch 后再调 model.get_rope_index ----
    print("\n[rope] ② 应用生产补丁 patch_qwen3_omni_rope_index_device() ...", flush=True)
    from relax.backends.megatron import patch_qwen3_omni_rope_index_device

    patch_qwen3_omni_rope_index_device()

    pos_ids, deltas = omni_model.get_rope_index(audio_seqlens=audio_seqlens_cuda, **call_kwargs)
    ...
    _chk("① 已复现 device 崩溃", reproduced, reproduced)
    _chk("② 补丁后不崩溃", True)
    _chk("pos_ids.device==cuda", pos_ids.device.type == "cuda", pos_ids.device)
    _chk("pos_ids.shape==[3,1,S]", tuple(pos_ids.shape) == (3, 1, len(out.tokens)), tuple(pos_ids.shape))
    _chk("deltas.device==cuda", deltas.device.type == "cuda", deltas.device)

    print("\n==== ROPE-INDEX SELFTEST RESULTS ====", flush=True)
    all_ok = True
    for name, ok, detail in checks:
        all_ok = all_ok and ok
        print(f"  [{'PASS' if ok else 'FAIL'}] {name}   {detail}", flush=True)
    if not all_ok:
        raise SystemExit("[rope] FAILED — 见上面 FAIL 项")
    print("[rope] ALL PASSED ✅（多段音频位置编码补丁在 GPU 上验证通过）", flush=True)


@app.local_entrypoint()
def selftest_rope() -> None:
    """GPU(T4) 最小测试：复现并验证「多段音频 get_rope_index device 补丁」，约 1-2 分钟。"""
    selftest_rope_index.remote()
